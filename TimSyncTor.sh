...
cat ${tempFastq1} ${tempFastq2} | sort -T ./ -k1,2 | awk -v pairedPrefix=${pairedPrefix} -v singlePrefix=${singlePrefix} 'BEGIN{OFS="\t"; FS="\t"; prevName=0; prevLine=0; prevRead=0; currentName=0; currentLine=0; currentRead=0};{currentName=$1; currentLine=$0; currentRead=$2; if(prevName!=0){if(prevName==currentName){file=pairedPrefix"r"prevRead".txt"; print prevLine > file; file=pairedPrefix"r"currentRead".txt";print currentLine > file; currentName=0}else{file=singlePrefix"r"prevRead".txt"; print prevLine > file}}; prevName=currentName; prevLine=currentLine; prevRead=currentRead};END{}'
